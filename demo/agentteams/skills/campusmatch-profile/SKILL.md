...
Call `mcporter list campusmatch --schema` if the tool contract is not already known. Then call `campusmatch.profile_materials` with `schema_version=1.0`, the assigned `task_id`, a stable `idempotency_key`, `user_id`, `source_id`, and the complete Markdown material.
